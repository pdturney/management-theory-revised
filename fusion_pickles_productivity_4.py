#
# Fusion Pickles Productivity 4 Parts
#
...
#
# row in table = <step number> <prod 4 0> <prod 3 1> <prod 2 2> 
#                <prod 1 3> <prod 0 4>
#
import golly as g
...
#
graph_file = fusion_dir + "/fusion_pickles_productivity_4.txt"
...
header = ["step num", \
  ...
  "error bars for 3 managers and 1 worker", \
  "error bars for 2 managers and 2 workers", \
  "error bars for 1 manager and 3 workers", \
...
for step_num in range(0, num_steps, step_size):
  ...
  if (len(growth_4m0w) > 1) and (step_num > 0):
    productivity_4m0w = np.mean(growth_4m0w)
    interval = st.t.interval(alpha=0.95, df=len(growth_4m0w)-1, \
               loc=np.mean(growth_4m0w), scale=st.sem(growth_4m0w))
    error_bar_4m0w = (interval[1] - interval[0]) / 2.0
  else:
    productivity_4m0w = 0.0
    error_bar_4m0w  = 0.0
  # calculate stats for 3 managers, 1 workers (3m1w)
  if (len(growth_3m1w) > 1) and (step_num > 0):
    productivity_3m1w = np.mean(growth_3m1w)
    interval = st.t.interval(alpha=0.95, df=len(growth_3m1w)-1, \
               loc=np.mean(growth_3m1w), scale=st.sem(growth_3m1w))
    error_bar_3m1w = (interval[1] - interval[0]) / 2.0
  else:
    productivity_3m1w = 0.0
    error_bar_3m1w  = 0.0
  # calculate stats for 2 manager,  2 worker (2m2w)
  if (len(growth_2m2w) > 1) and (step_num > 0):
    productivity_2m2w = np.mean(growth_2m2w)
    interval = st.t.interval(alpha=0.95, df=len(growth_2m2w)-1, \
               loc=np.mean(growth_2m2w), scale=st.sem(growth_2m2w))
    error_bar_2m2w = (interval[1] - interval[0]) / 2.0
  else:
    productivity_2m2w = 0.0
    error_bar_2m2w  = 0.0
  # calculate stats for 1 manager,  3 worker (1m3w)
  if (len(growth_1m3w) > 1) and (step_num > 0):
    productivity_1m3w = np.mean(growth_1m3w)
    interval = st.t.interval(alpha=0.95, df=len(growth_1m3w)-1, \
               loc=np.mean(growth_1m3w), scale=st.sem(growth_1m3w))
    error_bar_1m3w = (interval[1] - interval[0]) / 2.0
  else:
    productivity_1m3w = 0.0
    error_bar_1m3w  = 0.0
  # calculate stats for 0 managers, 4 workers (0m4w)
  if (len(growth_0m4w) > 1) and (step_num > 0):
    productivity_0m4w = np.mean(growth_0m4w)
    interval = st.t.interval(alpha=0.95, df=len(growth_0m4w)-1, \
               loc=np.mean(growth_0m4w), scale=st.sem(growth_0m4w))
    error_bar_0m4w = (interval[1] - interval[0]) / 2.0
  else:
    productivity_0m4w = 0.0
    error_bar_0m4w  = 0.0
  #
  ...
  graph_handle.write(
    ("{}\t{:.3f}\t{:.3f}\t{:.3f}\t{:.3f}\t{:.3f}\t{:.3f}" + 
    "\t{:.3f}\t{:.3f}\t{:.3f}\t{:.3f}\n").format(
    step_num, productivity_4m0w, productivity_3m1w, productivity_2m2w, 
    productivity_1m3w, productivity_0m4w, error_bar_4m0w, error_bar_3m1w, 
    error_bar_2m2w, error_bar_1m3w, error_bar_0m4w))
  #
#
graph_handle.close()
#
