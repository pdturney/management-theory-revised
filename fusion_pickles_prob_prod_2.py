#
# Fusion Pickles Probability * Productivity 2 Parts
#
...
#
# row in table = <step number> <p*p 2 0> <p*p 1 1> <p*p 0 2>
#
import golly as g
...
#
graph_file = fusion_dir + "/fusion_pickles_prob_prod_2.txt"
...
for step_num in range(0, num_steps, step_size):
  ...
  if (len(growth_2m0w) > 1) and (step_num > 0):
    probability_2m0w  = len(growth_2m0w) / num_seeds
    prob_prod_2m0w    = [probability_2m0w * growth for growth in growth_2m0w]
    mean_pp_2m0w      = np.mean(prob_prod_2m0w)
    interval = st.t.interval(alpha=0.95, df=len(prob_prod_2m0w)-1, \
               loc=np.mean(prob_prod_2m0w), scale=st.sem(prob_prod_2m0w))
  ...
  if (len(growth_1m1w) > 1) and (step_num > 0):
    probability_1m1w  = len(growth_1m1w) / num_seeds
    prob_prod_1m1w    = [probability_1m1w * growth for growth in growth_1m1w]
    mean_pp_1m1w      = np.mean(prob_prod_1m1w)
    interval = st.t.interval(alpha=0.95, df=len(prob_prod_1m1w)-1, \
               loc=np.mean(prob_prod_1m1w), scale=st.sem(prob_prod_1m1w))
  ...
  if (len(growth_0m2w) > 1) and (step_num > 0):
    probability_0m2w  = len(growth_0m2w) / num_seeds
    prob_prod_0m2w    = [probability_0m2w * growth for growth in growth_0m2w]
    mean_pp_0m2w      = np.mean(prob_prod_0m2w)
    interval = st.t.interval(alpha=0.95, df=len(prob_prod_0m2w)-1, \
               loc=np.mean(prob_prod_0m2w), scale=st.sem(prob_prod_0m2w))
    error_bar_0m2w = (interval[1] - interval[0]) / 2.0
  else:
    mean_pp_0m2w    = 0.0
    error_bar_0m2w  = 0.0
  #
  graph_handle.write("{}\t{:.3f}\t{:.3f}\t{:.3f}\t{:.3f}\t{:.3f}\t{:.3f}\n".format(
    step_num, mean_pp_2m0w, mean_pp_1m1w, mean_pp_0m2w, 
